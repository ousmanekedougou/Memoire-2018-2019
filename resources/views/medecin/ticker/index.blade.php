                ...
                <div class="row">
                  <div class="col-md-12">
                    ...
                    @if($ticker_client->prix  == $ticker_client->medecin->prix)
                      ...
                      <form id="delete-form-{{$ticker_client->id}}" action="{{ route('medecin.valider_ticker',$ticker_client->id) }}" method="post" style="display:none;">
                      @csrf
                      {{ method_field('PUT') }}
                    </form>
                      <span class="pull-right"> Si Le Patient a Payer en Liquide <a href=""  onClick=" if(confirm('Etes vous sure de valider ce ticker')){ event.preventDefault();document.getElementById('delete-form-{{$ticker_client->id}}').submit();}else{event.preventDefault();}" href="{{ route('medecin.update',$ticker_client->id) }}" class="btn btn-primary btn-xs"> <i class="fa fa-money"></i> Valider</a> </span>
                      @endif
                    </p>
                  </div>
                </div>
